...
const AuthForm: React.FC<AuthFormProps> = ({ type }) => {
  ...
  const handleSignInSubmit = async (data: SignInInput) => {
    ...
    try {
      ...
      if (result?.success) {
        toast.add({ title: "Welcome back!", description: "You have successfully signed in.", type: "success" });
        setIsSubmitted(true);
      } else {
        toast.add({ title: "Sign in failed", description: result?.error || "Invalid credentials.", type: "error" });
      }
    } catch {
  ...
  const handleUserSignUpSubmit = async (data: SignUpUserInput) => {
    ...
    try {
      ...
      if (result?.success) {
        toast.add({ title: "Account created!", description: "Welcome to Cheerity!", type: "success" });
        setIsSubmitted(true);
      } else {
        toast.add({ title: "Sign up failed", description: result?.error || "An error occurred.", type: "error" });
      }
    } catch {
      toast.add({ title: "Error", description: "Something went wrong.", type: "error" });
    }
  ...
  const handleInstitutionSignUpSubmit = async (data: SignUpInstitutionInput) => {
    ...
    try {
      ...
      if (result?.success) {
        toast.add({ title: "Application submitted!", description: "Our admin team will review your institution.", type: "success" });
        setIsSubmitted(true);
      } else {
        toast.add({ title: "Registration failed", description: result?.error || "An error occurred.", type: "error" });
      }
    } catch {
      toast.add({ title: "Error", description: "Something went wrong.", type: "error" });
    }
  ...
  if (isSubmitted) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center">
        <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full" style={{ background: "var(--color-primary-100)", color: "var(--color-primary)" }}>
          <Check className="h-8 w-8" />
        </div>
        <h2 className="text-2xl font-bold mb-2" style={{ fontFamily: "var(--font-poppins)" }}>
          {isSignIn ? "Welcome Back!" : role === "institution" ? "Application Submitted!" : "Account Created!"}
        </h2>
        <p className="text-sm max-w-md mb-6" style={{ color: "var(--color-on-surface-variant)" }}>
          {isSignIn
            ? "You have successfully signed in to Cheerity."
            : role === "institution"
            ? "Your institution account application has been submitted. Our admin team will review and approve your account shortly."
            : "Your account has been successfully created. Welcome to the Cheerity community!"}
        </p>
        <Link
          href="/"
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {!isSignIn && (
        <div className="role-toggle">
          ...
          <button
            ...
            className={`role-toggle-btn ${role === "institution" ? "active" : ""}`}
          >
        ...
        <form onSubmit={signInForm.handleSubmit(handleSignInSubmit)} className="flex flex-col gap-4">
          ...
          <div className="form-group">
            ...
            {signInForm.formState.errors.email && (
              <p className="form-error">{signInForm.formState.errors.email.message}</p>
            )}
          </div>
          ...
          <div className="form-group">
            ...
            {signInForm.formState.errors.password && (
              <p className="form-error">{signInForm.formState.errors.password.message}</p>
            )}
          </div>

          {/* Forgot Password */}
          <div className="flex justify-end">
            <span className="text-sm font-medium cursor-pointer" style={{ color: "var(--color-primary)" }}>
              Forgot Password?
            </span>
          </div>
        ...
        <form onSubmit={userSignUpForm.handleSubmit(handleUserSignUpSubmit)} className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="form-group">
              ...
              <input type="tel" placeholder="Enter your phone number" {...userSignUpForm.register("phonenumber")} className="form-input" />
          ...
          <div className="form-group">
            <label className="form-label">Email Address</label>
            <input type="email" placeholder="Enter your email address" {...userSignUpForm.register("email")} className="form-input" />
          ...
          <div className="form-group">
            ...
            <input type="text" placeholder="Enter your full address" {...userSignUpForm.register("address")} className="form-input" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="form-group">
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2" style={{ color: "var(--color-on-surface-variant)", background: "none", border: "none", cursor: "pointer" }}>
                  {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              </div>
            ...
            <div className="form-group">
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 -translate-y-1/2" style={{ color: "var(--color-on-surface-variant)", background: "none", border: "none", cursor: "pointer" }}>
        ...
        <form onSubmit={institutionSignUpForm.handleSubmit(handleInstitutionSignUpSubmit)} className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="form-group">
              ...
              <input type="email" placeholder="Enter email address" {...institutionSignUpForm.register("email")} className="form-input" />
            ...
            <div className="form-group">
              <label className="form-label">Phone Number</label>
              <input type="tel" placeholder="Enter phone number" {...institutionSignUpForm.register("phoneNumber")} className="form-input" />
          ...
          <div className="form-group">
            ...
            <input type="text" placeholder="Enter your full address" {...institutionSignUpForm.register("address")} className="form-input" />
          ...
          {/* Pickup Service */}
          <div className="rounded-2xl p-4" style={{ background: "var(--color-primary-50)", border: "1px solid var(--color-primary-100)" }}>
            <label className="form-label mb-2 block">Do you provide pickup service for nearby donors?</label>
            <div className="flex items-center gap-6">
              <label className="flex items-center gap-2 cursor-pointer text-sm font-medium">
                <input type="radio" value="yes" {...institutionSignUpForm.register("hasPickupService")} className="h-4 w-4" style={{ accentColor: "var(--color-primary)" }} />
                <span>Yes, we offer pickup</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer text-sm font-medium">
                <input type="radio" value="no" {...institutionSignUpForm.register("hasPickupService")} className="h-4 w-4" style={{ accentColor: "var(--color-primary)" }} />
                <span>No, drop-off only</span>
              </label>
            </div>
          </div>
          ...
          <div className="form-group">
            <label className="form-label">
              Official Website or Social Media <span style={{ color: "var(--color-on-surface-variant)", fontWeight: 400 }}>(Optional)</span>
            </label>
            <input type="text" placeholder="https://..." {...institutionSignUpForm.register("websiteOrSocial")} className="form-input" />
          </div>

          {/* Description */}
          <div className="form-group">
            <label className="form-label">Short Description about your Institution</label>
            <textarea
              rows={3}
              placeholder="Describe your institution's mission, programs, and target beneficiaries..."
      ...
      {/* Switch auth page link */}
      <div className="text-center pt-2 text-sm" style={{ color: "var(--color-on-surface-variant)" }}>
        {isSignIn ? (
          <p>
            Don&apos;t have an account?{" "}
            <Link href="/sign-up" className="font-bold" style={{ color: "var(--color-primary)" }}>
              Sign Up
            </Link>
          </p>
        ) : (
          <p>
            ...
            <Link href="/sign-in" className="font-bold" style={{ color: "var(--color-primary)" }}>
              Sign In
            </Link>
          </p>
        )}
